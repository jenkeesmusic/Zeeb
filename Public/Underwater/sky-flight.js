...
export function sampleFlight(time,entry=WHIRLPOOL,reducedMotion=false) {
  ...
  return {x,y,z,phase,done:t>=FLIGHT_DURATION,
    // Six full turns (one with reduced motion) settle face-forward at the summit.
    spin:Math.PI*(reducedMotion?2:12)*flightEase(t/15),
    rocket:flightEase((t-9.5)/1)*(1-flightEase((t-15.5)/1)),
    rocketPass:clamp((t-9.5)/7),
    canopy:flightEase((t-18.65)/.9)*(1-flightEase((t-39.2)/1.2)),
