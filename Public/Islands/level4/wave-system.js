// Gerstner Wave System for Zeeb Islands — Level 2 (Night Mode)
...
class GerstnerWaveSystem {
    constructor() {
        ...
        // Warm orange-gold target. Was yellow (255,230,50), but yellow over
        // blue water mixes GREEN — a redder gold keeps the water blue with
        // warm light on it instead.
        this.flushAmber = { r: 255, g: 165, b: 80 };
    }

    /**
     * Initialize wave layers with dark night ocean colors.
    ...
    init() {
        this.layers = [
            ...
            new GerstnerWaveLayer({
                ...
                crestColor: '#234a7c',
                deepColor: '#0d2142',
                foamColor: 'rgba(180, 200, 220, 0.25)',
                alpha: 1,
                yPosition: 0.55,
                speed: 1,
                components: [
    ...
    // Level 4 sunrise: the game sets this.sunrise (0-1) each frame and the
    // dawn light warms the water through the same amber blend. Kept subtle
    // (0.28 max) so the ocean stays blue — the gold sun-path sheen in
    // drawOceanFill carries most of the sunrise-on-water look.
    getFlushIntensity() {
        const dawn = (this.sunrise || 0) * 0.28;
        if (this.flushTimer <= 0) return dawn;
        const t = this.flushTimer / this.flushDuration; // 1→0
        // Quick peak then gradual fade
        return Math.max(dawn, t * t);
    }
